...
def build_loader_finetune(config, logger):
    ...
    #dataset_train = CartesianFastMRIDatasetBuilder(path='/rfanfs/pnl-zorro/projects/fastmri/data/multicoil_val').preprocessed_ds
    config.freeze()
...
def build_dataset(is_train, config, logger):
    ...
    if config.DATA.DATASET == 'imagenet':
       ...
       root = '/rfanfs/pnl-zorro/projects/fastmri/images'
       root = os.path.join(config.DATA.DATA_PATH, prefix)
       '''
       def pil_loader(path):
           with open(path, "rb") as f:
               ...
               return img
       '''
       #dataset = datasets.ImageFolder(root, loader=lambda path: pil_loader(path), transform=transform)
...
def build_transform(is_train, config):
    ...
    if resize_im:
        if config.TEST.CROP:
            ...
            print('else')
            t.append(
                transforms.Resize((4, config.DATA.IMG_SIZE, config.DATA.IMG_SIZE),
                                  interpolation=str_to_pil_interp(config.DATA.INTERPOLATION))
            )
    #transform = SimMIMTransform(config)
    t.append(transforms.Lambda(lambda img: img.convert('RGBA') if img.mode != 'RGBA' else img))
    t.append(transforms.ToTensor())
    #t.append(transforms.Normalize([0.5073, 0.5073, 0.5073], [0.2534, 0.2534, 0.2534]))
